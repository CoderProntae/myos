...
static void cmd_help(void) {
    ...
    vga_print("- Yardim\n");
    vga_print_colored("  clear    ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Ekrani temizle\n");
    vga_print_colored("  about    ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Hakkinda\n");
    vga_print_colored("  echo     ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Yazi yazdir\n");
    vga_print_colored("  color    ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Renk paleti\n");
    vga_print_colored("  time     ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Saat\n");
    vga_print_colored("  sysinfo  ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Sistem bilgisi\n");
    vga_print_colored("  reboot   ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Yeniden baslat\n");
    vga_print_colored("  shutdown ", VGA_LIGHT_CYAN, VGA_BLACK);
    vga_print("- Kapat\n\n");
}
...
static void cmd_about(void) {
    ...
    vga_print_colored("         |___/              \n\n", VGA_BLUE, VGA_BLACK);
    vga_set_color(VGA_WHITE, VGA_BLACK);
    vga_print("  Versiyon : 0.1.0\n");
    vga_print("  Mimari   : x86 32-bit\n");
    vga_print("  Dil      : C + Assembly\n\n");
}
...
static void process_command(const char* cmd) {
    ...
    if (k_strcmp(cmd, "cls") == 0)      { vga_clear(); return; }
    if (k_strcmp(cmd, "about") == 0)    { cmd_about(); return; }
    if (k_strcmp(cmd, "color") == 0)    { cmd_color(); return; }
    if (k_strcmp(cmd, "time") == 0)     { cmd_time(); return; }
    if (k_strcmp(cmd, "sysinfo") == 0)  { cmd_sysinfo(); return; }
    if (k_strcmp(cmd, "reboot") == 0)   { outb(0x64, 0xFE); return; }
